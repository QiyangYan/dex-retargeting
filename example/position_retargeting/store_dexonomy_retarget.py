...
def get_hand_surface_points_from_handmodel(
    ...
    Args:
        ...
        urdf_path: URDF 文件路径
        mesh_path: Mesh 文件路径
        num_samples: 采样点数（通过调整每个 link 的采样数控制，默认约 2000）
        device: 计算设备
        return_normals: 是否返回法向量（默认 True）
    ...
    try:
        ...
        rot_matrix = rotation_scipy.as_matrix()
        rot6d = rot_matrix[:, :2].flatten()  # (6,)
        
        # 组装 q 格式
        q = np.concatenate([translation, rot6d, joint_angles])
        q_tensor = torch.from_numpy(q).unsqueeze(0).float().to(torch_device)
        
        # 3. 更新运动学
        global_translation = q_tensor[:, :3]
        global_rotation = robust_compute_rotation_matrix_from_ortho6d(q_tensor[:, 3:9])
        current_status = robot.forward_kinematics(q_tensor[:, 9:])
        ...
        if current_num != num_samples:
            if current_num > num_samples:
                ...
                cprint(f"[INFO] Downsampled from {current_num} to {num_samples} points", "white")
            else:
                # 上采样：重复采样
                ...
                if return_normals:
                    surface_normals = surface_normals[indices]
                cprint(f"[INFO] Upsampled from {current_num} to {num_samples} points", "white")
        ...
        cprint(f"[INFO] Extracted {surface_points_np.shape[0]} surface points (target: {num_samples})", "green")
            ...
            try:
                ...
                try:
                    ...
                    if use_handmodel_method:
                        ...
                        try:
                            hand_surface_points, hand_surface_normals = get_hand_surface_points_from_handmodel(
                            ...
                            hand_surface_points, hand_surface_normals = get_hand_surface_points_from_sapien(
                                viewer, 
                                robot_idx=0,
                                num_samples=hand_surface_sample_num,
                                return_normals=True
                ...
                retargeted_poses_dict[continuous_idx] = {
                    ...
                    # 手部数据
                    "hand_surface_points": hand_surface_points if hand_surface_points is not None else np.array([]),  # 手部表面点 (N, 3)
                    "hand_surface_normals": hand_surface_normals if hand_surface_normals is not None else np.array([]),  # 手部表面法向量 (N, 3)
                    # 物体数据
                ...
                if visualize:
                    ...
                    viewer.set_shadow_qpos(grasp_qpos, robot_idx=0, y_offset=0.0)
                    # 设置 omni hand 的 qpos
                    if len(retargeted_poses_dict[continuous_idx]["robot_pose"]) > 0:
